...
def imperial_str_to_float(measure: str) -> float:
    ...
    assert is_safe(measure)
    return float(eval(measure.strip().replace(" ", "+")) * IN)
...
class Thread(Solid):
    ...
    def __init__(
        ...
        if not simple:
            ...
            cq_object = self.chamfer_ends(cq_object)  # type: ignore
    ...
    def make_thread_with_faded_ends(
        ...
        faces: list[cq.Face] = thread_faces
        if number_faded_ends == 2:
            assert fade_faces_bottom
            assert fade_faces_top
            faces += [cast(cq.Face, x) for x in fade_faces_bottom + fade_faces_top]

        elif self.end_finishes[0] == "fade":
            assert fade_faces_bottom
            faces += [cast(cq.Face, x) for x in fade_faces_bottom + [end_faces[1]]]

        else:
            assert fade_faces_top
            faces += [cast(cq.Face, x) for x in fade_faces_top + [end_faces[0]]]

        thread_shell = cq.Shell.makeShell(faces)
    ...
    def chamfer_ends(self, cq_object: Solid):
        ...
        if self.end_finishes.count("chamfer") != 0:
            ...
            chamfered = cq_object.intersect(cast(Solid, cutter.val()))
        return chamfered
