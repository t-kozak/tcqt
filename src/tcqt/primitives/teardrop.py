# Credits https://github.com/CadQuery/cadquery-plugins/blob/main/plugins/teardrop/teardrop.py
import math
...
def teardrop(
    workplane: cq.Workplane, radius: float = 1, rotate: float = 0, clip: Optional[float] = None
) -> cq.Workplane:
    """
    ...
    if clip is None:
        ...
        raise ValueError(f"teardrop - value of 'clip' argument must be less than {ymax}")

    elif clip <= -radius:
        raise ValueError(f"teardrop - value of 'clip' argument must be greater than {-radius}")
    ...
    return workplane.eachpoint(lambda loc: w.moved(loc), True)


# Patch the function into the Workplane class
cq.Workplane.teardrop = teardrop  # type: ignore
